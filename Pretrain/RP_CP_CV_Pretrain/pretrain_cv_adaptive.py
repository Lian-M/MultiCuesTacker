...
X_cb_cur_reshape = np.expand_dims(X_cb_cur_reshape, axis=1) # (B*N, 1, 4)
PC_tempin = np.concatenate((X_cb_past_reshape, X_cb_cur_reshape), axis=1)   # (B*N, 7, 4)

total_iou = np.zeros((B*N, 6))
for b in range(B*N):
    for t in range(6):
        total_iou[b, t] = compute_iou_array(PC_tempin[b, t, :], PC_tempin[b, t+1, :])
total_iou_past = np.zeros((B*N, 5))
total_iou_cur = np.zeros((B*N, 1))
for b in range(B*N):
	total_iou_past[b, :] = total_iou[b, :5]
	total_iou_cur[b, :] = total_iou[b, 5]


X_cb_past_reshape1 = np.expand_dims(total_iou_past, axis=-1) # (B*N, 5, 1)
X_cb_cur_reshape1 = total_iou_cur                            # (B*N, 1)
...
# Build Model
model = LSTMtracker(ninp[1], nhid[1], nout[1], nlayers[1])
...
for epoch in trange(num_epochs, desc="Training", unit="epoch"):
	...
	scheduler.step()
	validate_one_epoch()
torch.save(model.state_dict(), 'camera_v_weights.pt')
